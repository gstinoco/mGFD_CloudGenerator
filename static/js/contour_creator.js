...
function updateUploadContent(state) {                                                                                                  // Declare function scope logic
    ...
    switch (state) {                                                                                                                   // Evaluate switch condition check flow
        case 'drag-over':                                                                                                              // Define switch case branch logic
            uploadTitle.textContent = window.I18N.dropImageHere || 'Drop your image here';
        ...
        default:                                                                                                                       // Execute sequential evaluation stream node
            uploadTitle.textContent = window.I18N.dropImageHere || 'Drop your image here';
            uploadSubtitle.textContent = window.I18N.clickToBrowse || 'or click to browse files';
            break;                                                                                                                     // Terminate current loop block context
    }                                                                                                                                  // Terminate block scope execution context
}                                                                                                                                      // Terminate block scope execution context
...
function showUploadProgress(file) {
    ...
    if (progressMessage) {
        progressMessage.textContent = (window.I18N && window.I18N.uploadingFile) ? 
            window.I18N.uploadingFile.replace('{filename}', file.name) : `Uploading ${file.name}...`;
    }
}                                                                                                                                      // Terminate block scope execution context
...
function showUploadSuccess(file) {
    ...
    if (uploadedFileName) {
        uploadedFileName.textContent = `${file.name} (${Utils.formatFileSize(file.size)})`;
    }
}                                                                                                                                      // Terminate block scope execution context
...
function uploadFile() {                                                                                                                // Declare function scope logic
    ...
    if (!file) {                                                                                                                       // Evaluate boolean condition check logic
        showAlert(window.I18N.pleaseSelectFile || 'Please select a file', 'error');
        return;                                                                                                                        // Execute sequential statement instruction block
    }                                                                                                                                  // Terminate block scope execution context
    ...
    fetch(`${BASE_URL}/upload`, {                                                                                                      // Dispatch asynchronous network payload stream
        ...
        .then(data => {                                                                                                                // Chain asynchronous promise resolution sequence
            if (data.success) {                                                                                                        // Evaluate boolean condition check logic
                ...
                showAlert(window.I18N.imageUploadedSuccessfully || 'Image uploaded successfully', 'success');
        ...
        .catch(error => {                                                                                                              // Catch asynchronous promise exception payload
            showAlert((window.I18N.errorUploadingFile || 'Error uploading file: ') + error.message, 'error');
        });                                                                                                                            // Terminate block scope execution context
}                                                                                                                                      // Terminate block scope execution context
...
function handleMouseDown(event) {                                                                                                      // Declare function scope logic
                                                                                                                                       // Allow drag with Ctrl+Click (Mac right-click) or Middle Click
    if (event.ctrlKey || event.button === 1) {                                                                                         // Evaluate boolean condition check logic
...
function detectRegion(x, y) {
    ...
    if (window.showGlobalLoading) window.showGlobalLoading(window.I18N.detectingRegions || 'Detecting regions...');
    ...
    fetch(`${BASE_URL}/detect_region`, {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                showAlert(window.I18N.regionDetected || 'Region detected! Click "Add Region" to confirm.', 'success');
            } else {
        ...
        .catch(error => {
            if (window.hideGlobalLoading) window.hideGlobalLoading();
            showAlert(`${window.I18N.errorDetecting || 'Error detecting region:'} ${error.message}`, 'error');
        });
}                                                                                                                                      // Terminate block scope execution context
...
function updateRegionsList() {                                                                                                         // Declare function scope logic
    ...
    if (detectedRegions.length === 0) {
        ...
        regionsList.innerHTML = `
            <div class="col-span-full flex flex-col items-center justify-center py-16 px-4 text-center bg-[#111] rounded-2xl border border-white/5 border-dashed">
                ...
                <h4 class="text-xl font-bold text-white mb-2">${window.I18N.noRegionsTitle || 'No regions detected'}</h4>
                <p class="text-gray-400 max-w-md">${window.I18N.noRegionsDesc || 'Click on the image to detect and add contour regions'}</p>
            </div>
        `;
        return;
    }
    ...
    regionsList.innerHTML = detectedRegions.map((region, index) => {
        ...
        return `
            <div class="bg-[#111] rounded-2xl border ${region.visible ? 'border-white/10' : 'border-white/5 opacity-60'} overflow-hidden shadow-xl hover:border-white/20 transition-all flex flex-col h-full group">
                <!-- Header -->
                <div class="p-4 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                    <div class="flex items-center gap-3">
                        <div class="w-3 h-3 rounded-full" style="background: ${region.color}; box-shadow: 0 0 10px ${region.color}80;"></div>
                        <h4 class="font-bold text-white truncate max-w-[120px] md:max-w-[160px]" title="${region.name}">${region.name}</h4>
                    </div>
                    <span class="text-xs font-mono bg-white/5 border border-white/10 text-gray-300 px-2.5 py-1 rounded-md">${pointCount} pts</span>
                </div>
                
                <!-- Body -->
                <div class="p-4 flex-1">
                    <div class="flex items-center justify-between mb-3 text-sm">
                        <span class="text-gray-400 font-medium"><i class="fas fa-compress-arrows-alt text-blue-400/80 mr-1.5"></i> ${window.I18N.nodeRetention || 'Node Retention'}</span>
                        <span id="smoothing-val-${index}" class="font-mono text-white bg-blue-500/10 text-blue-300 px-2 rounded">${(region.smoothing !== undefined ? region.smoothing : 100).toFixed(2)}%</span>
                    </div>
                    
                    <div class="flex items-center gap-4">
                        <input type="range" id="regionDensity_${index}" class="flex-1 accent-blue-500 h-1.5 bg-gray-800 rounded-lg appearance-none cursor-pointer"
                        ...
                        <div class="flex items-center gap-1.5 bg-black/40 px-2 py-1 rounded-lg border border-white/10">
                            <input type="number" id="regionDensityExact_${index}" 
                                class="w-12 bg-transparent text-center text-white font-mono text-sm outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            ...
                            <span class="text-xs text-gray-500 font-medium">${window.I18N.nodesText || 'nodes'}</span>
                        </div>
                    </div>
                </div>
                
                <!-- Actions -->
                <div class="grid grid-cols-3 divide-x divide-white/5 border-t border-white/5 bg-black/20">
                    <button class="py-3 flex items-center justify-center text-gray-500 hover:text-white hover:bg-white/5 transition-colors ${region.visible ? 'text-blue-400 hover:text-blue-300' : ''}" 
                        onclick="toggleRegion(${index})" title="${region.visible ? 'Hide' : 'Show'} region">
                        <i class="fas ${region.visible ? 'fa-eye' : 'fa-eye-slash'}"></i>
                    </button>
                    <button class="py-3 flex items-center justify-center text-gray-500 hover:text-teal-400 hover:bg-teal-400/10 transition-colors" 
                        onclick="exportSingleRegion(${index})" title="Export region">
                        <i class="fas fa-download"></i>
                    </button>
                    <button class="py-3 flex items-center justify-center text-gray-500 hover:text-red-400 hover:bg-red-400/10 transition-colors" 
                        onclick="deleteRegion(${index})" title="Delete region">
...
function showAlert(message, type) {
    if (typeof window.showToast === 'function') {
        const title = type === 'success' ? (window.I18N.successTitle || 'Success') : (type === 'error' ? (window.I18N.errorTitle || 'Error') : (window.I18N.notificationTitle || 'Notification'));
...
function showFloatingNotification(message, type = 'info', duration = 5000) {                                                           // Declare function scope logic
    ...
    const cssClasses = typeClasses[type] || typeClasses['info'];

    notification.innerHTML = `
        <div class="flex items-center gap-4 px-6 py-4 rounded-2xl border backdrop-blur-md shadow-2xl pointer-events-auto ${cssClasses} animate-[fade-in-up_0.3s_ease-out_forwards]">
            <i class="fas ${getNotificationIcon(type)} text-lg"></i>
            <span class="font-medium text-white">${message}</span>
            <button class="ml-4 text-gray-500 hover:text-white transition-colors" onclick="closeNotification(this)">
...
let brushSize = 10;                                                                                                                    // Initialize mutable variable state reference
...
function updateBrushSize(size) {                                                                                                       // Declare function scope logic
    ...
    document.getElementById('brushSizeValue').textContent = size + 'px';                                                               // Query document element node reference
}                                                                                                                                      // Terminate block scope execution context
...
async function applyRefinements() {                                                                                                    // Declare asynchronous execution wrapper context
    ...
    try {
        if (window.showGlobalLoading) window.showGlobalLoading(window.I18N.applyingRefinement || 'Applying brush refinement...');
        ...
        if (result.success) {
            ...
            brushStrokes = [];
            updateRefineButtons();
            showAlert(window.I18N.refinementApplied || 'Refinement applied successfully.', 'success');
        } else {
            showAlert(`${window.I18N.errorApplying || 'Error applying refinement:'} ${result.error}`, 'error');
        }
